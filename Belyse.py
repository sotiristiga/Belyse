...
def metrics_customize(iconname,sline,i):
    htmlstr = f"""<p style='background-color: rgb(0,204,102, 0.75); 
...
with kpi1:
    st.markdown(lnk + metrics_customize("fas fa-shopping-cart","Ολοκληρωμένες Παραγγελίες",belyse_complete_orders['id_order'].nunique()), unsafe_allow_html=True)
with kpi1:
    st.markdown(lnk + metrics_customize("far fa-lightbulb","Συνολικά προϊόντα που πουλήθηκαν",belyse_complete_orders['id'].nunique()), unsafe_allow_html=True)

kpi2.metric(label="Aκυρωμένες Παραγγελίες",
        value=belyse[belyse['order_situation']=="Ακυρωμένη"][['id_order']].nunique())
kpi3.metric(label="Παραγγελίες που απορρίφθηκαν",
        value=belyse[belyse['order_situation']=="Απορρίφθηκε"][['id_order']].nunique())
kpi4.metric(label="Επιστροφή Προϊόντών",
        value=belyse['Return'].sum())
kpi5.metric(label="Έσοδα απο πωλήσεις",
        value=belyse_complete_orders['Total_payment'].sum().round(2))
kpi6.metric(label="Προμήθεια Skroutz",
        value=belyse_complete_orders.groupby('id_order')['Skroutz_Commission'].mean().reset_index()['Skroutz_Commission'].sum().round(2))
kpi7.metric(label="Αρ. προϊόντων ανά παραγγελία",
        value=belyse_complete_orders['id_order'].value_counts().reset_index()['count'].mean().round(1))

top41, top42 = st.columns(2)
with top41:
    ...
    fig_line_comp_orders_mbm.update_traces(textfont_size=16, textangle=0, marker_color='#8690FF', cliponaxis=False,
                                           textposition='inside')
...
with tab2:
    ...
    with tab21:
        fig_line_comp_orders_dbd = px.line(
            belyse_complete_orders.groupby(['Date_order', 'id_order'])['Price'].mean().reset_index().groupby(
                'Date_order')['Price'].sum().reset_index().sort_values('Date_order'),
    ...
    with tab22:
        fig_line_comp_orders_mbm = px.bar(
            belyse_complete_orders.groupby(['Month_Year', 'id_order'])['Price'].mean().reset_index().groupby(
                'Month_Year')['Price'].sum().reset_index().sort_values('Month_Year'),
        ...
        st.write(fig_line_comp_orders_mbm)
    with tab23:
        fig_line_comp_orders_yby = px.line(belyse_complete_orders.groupby(['Month_order', 'Year_order'])['Price'].sum().reset_index().sort_values(
                'Month_order'),
...
with col2:
    fig_line_comp_orders_mbm = px.bar(
        belyse_complete_orders.groupby('Supplier')['Price'].sum().reset_index().sort_values('Price'),
        x="Supplier", y="Price",
        title='Συνολική παραγωγή σε πωλήσεις',
        labels={'Supplier': 'Προμηθευτής', "Price": 'Πωλήσεις €'},
        text='Price', width=1000)
